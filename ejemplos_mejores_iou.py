...
# ─────────────────────────── 6) BÚSQUEDA DE MEJORES IoU ────────────────────
# Inicializa con IoU=0.0 para cada clase
best_iou = {c: (0.0, None) for c in range(6)}
...
for img_path in sorted(IMAGE_DIR.iterdir()):
    ...
    # --- Actualiza máximos por clase (primera vez que asciende) ---
    for c, iou in enumerate(ious):
        if np.isnan(iou):
            continue
        prev_iou, prev_path = best_iou[c]
        if iou > prev_iou:
            best_iou[c] = (float(iou), str(img_path))

# ─────────────────────────── 7) RESULTADOS ────────────────────────────────
print("\n>> Imagen con mejor IoU por clase")
for c in range(6):
    score, path = best_iou[c]
    label = CLASS_NAMES[c]
    if path is None:
