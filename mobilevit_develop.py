...
Se añade un módulo de atención CBAM en el cuello de botella ASPP.
"""
...
from tensorflow.keras.layers import (
    ...
    Dense, Multiply, Reshape, GlobalAveragePooling2D, GlobalMaxPooling2D, Add
)
...
# Módulo de atención de canal (Channel Attention)
def channel_attention_module(x, ratio=8):
    """
    Módulo de atención de canal (Channel Attention) de CBAM, implementado en Keras.
    """
    channel_axis = -1
    filters = x.shape[channel_axis]

    # Capas compartidas para el MLP (implementado con Dense para eficiencia)
    shared_layer_one = Dense(filters // ratio, activation='relu', use_bias=False)
    shared_layer_two = Dense(filters, use_bias=False)

    # Rama de Average Pooling
    ...
    attention = Activation('sigmoid')(cbam_feature)
    
    return Multiply()([x, attention])

# Módulo de atención espacial (Spatial Attention)
def spatial_attention_module(x, kernel_size=7):
    """
    Módulo de atención espacial (Spatial Attention) de CBAM, implementado en Keras.
    """
    channel_axis = -1
    ...
    attention = Conv2D(1, kernel_size, padding='same', activation='sigmoid', use_bias=False)(concat)
    
    return Multiply()([x, attention])

def ASPP_with_CBAM(x, dilation_rates=[6, 12, 18]):
    """
    Módulo ASPP (Atrous Spatial Pyramid Pooling) mejorado con atención CBAM.
    """
    input_shape = x.shape
    ...
    # Aplicación de los módulos CBAM
    cbam_features = channel_attention_module(concatenated, ratio=16)
    cbam_features = spatial_attention_module(cbam_features, kernel_size=7)

    projected = conv_block(cbam_features, projected_filters, 1)
    return projected
...
def build_mobilevit_model(shape=(256, 256, 3), num_classes_arg=None):
    ...
    y un decodificador U-Net con skip connections y ASPP+CBAM en el cuello de botella.
    """
    ...
    # --- Cuello de botella con ASPP + CBAM ---
    # Se utiliza la nueva función ASPP_with_CBAM en lugar de ASPP_mejorado
    x = ASPP_with_CBAM(bottleneck, dilation_rates=[2, 4, 6]) # 8x8 -> 8x8
...
with tf.device(device):
    ...
    iou_aware_model.fit(train_X, train_y, batch_size=8, epochs=50, # Batch size más grande gracias a la eficiencia
