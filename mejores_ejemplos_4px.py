"""
Encuentra, para cada clase (0–5), la primera imagen con el mejor IoU (más alto)
...
# ─────────────────────────── 7) RESULTADOS ────────────────────────────────
cls = 4  # “Otro”
iou_list = []  # aquí almacenaremos (iou, ruta)
...
for img_path in sorted(IMAGE_DIR.iterdir()):
    ...
    gt_idx    = resize_nearest(load_gt_idx(mask_path),
    ...
    # IoU por clase
    ious = iou_per_class(pred_idx, gt_idx, 6)
    iou_val = ious[cls]
    if not np.isnan(iou_val):
        iou_list.append((float(iou_val), str(img_path)))

# ─────────────────────────── 4) Ordena y toma el segundo mejor ──────────────
# Orden descendente por IoU
iou_list_sorted = sorted(iou_list, key=lambda x: x[0], reverse=True)

if len(iou_list_sorted) >= 2:
    segundo_iou, segundo_path = iou_list_sorted[1]
    print(f"Segundo mejor IoU para clase {cls} ({CLASS_NAMES[cls]}): "
          f"IoU = {segundo_iou:.4f}  |  {segundo_path}")
else:
    print("No hay suficientes ejemplos válidos para la clase 4.")