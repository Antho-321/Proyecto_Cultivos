...
class_weights = np.array([0.2012, 5.9698, 11.5819, 7.2159, 27.0968, 1.6638],
                         dtype=np.float32)
weights_dict_silent = {i: float(w) for i, w in enumerate(class_weights)}
...
def focal_loss(alpha=None, gamma=2.0):
    def loss(y_true, y_pred):
        # Your data loading already ensures y_true is rank-3 ([batch, H, W]).
        # We can remove the conditional squeeze entirely.
...
loss_focal = weighted_focal_lovasz_loss(weights_dict_silent, gamma=2.0)
