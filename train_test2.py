...
def train_fn(loader, model, optimizer, loss_fn, scaler):
    ...
    for batch_idx, (data, targets) in enumerate(loop):
        data = (
            data.to(Config.DEVICE, non_blocking=True)
                .to(memory_format=torch.channels_last)    # ✨ new
        )
...
def main():
    ...
    torch.backends.cuda.matmul.allow_tf32 = True        # matmul (GEMM)
    ...
    train_loader = DataLoader(
        ...
        prefetch_factor    = 4         # ✨ new (2 is default)
    )
    ...
    model = CloudDeepLabV3Plus(num_classes=6).to(
        Config.DEVICE,
        memory_format=torch.channels_last            # ✨ new
    )
