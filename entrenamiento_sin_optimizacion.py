...
# ====================================================================
# 2. TRAIN Y VALIDACIÓN (igual que antes)
# ====================================================================
...
def check_metrics(loader, model, n_classes: int = 6, device: str = "cuda", use_amp: bool = True):
    ...
    for x, y in loader:
        ...
        preds = logits.argmax(1)
        hist = torch.bincount((preds * n_classes + y).view(-1),
                              minlength=n_classes * n_classes).view(n_classes, n_classes)
...
def main(resume: bool = True):
    ...
    if resume and os.path.isfile(Config.MODEL_SAVE_PATH):
        print("📥 Cargando checkpoint para reanudar entrenamiento …")
    ...
    # Evaluar mejor checkpoint final
