...
def dense_crf_refine(image_rgb, probs, n_iters: int = 50):
    """Refina softmax C×H×W con Dense CRF; devuelve máscara H×W."""
...
def morph_refine(mask, k: int = 3, min_area: int = 50):
    """Opening + Closing y filtrado de componentes pequeñas."""
...
def postprocess(logits, rgb_img):
    """logits: torch.Tensor C×H×W (cpu); rgb_img: np.ndarray H×W×3"""
...
def run_inference():
    ...
    for i, (img_t, gt_mask) in enumerate(loader):
        ...
        rgb = np.array(Image.open(os.path.join(Config.VAL_IMG_DIR,
                                               val_ds.images[i])))

        refined = postprocess(logits, rgb)                  # H×W uint8

        # ---- guardar
        out_pth = os.path.join(SAVE_DIR,
                               f"{os.path.splitext(val_ds.images[i])[0]}_pp.png")
